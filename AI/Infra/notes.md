...
## 版图速查：端侧 Infra vs 在线服务 Infra

> 学习路线导航，帮你区分两条路，避免什么都学。

AI Infra 实际分两条主线，技术栈、技能底座、职业方向都不同：

| 维度 | 端侧 / 离线推理 | 在线服务 |
|------|----------------|----------|
| 核心问题 | 单次推理跑得快、跑得省 | 同时服务很多人跑得顺 |
| 典型场景 | Jetson、车载、手机、单卡离线批处理 | API 服务、高并发生成 |
| 关键技术 | TensorRT、CUDA/Triton、ONNX、量化 | vLLM、SGLang、TensorRT-LLM、调度 |
| 技能底座 | C++、Linux、算子、硬件 | Python、分布式、网络、容器 |
| 本笔记相关章节 | §2-5、§9、§11、§14 | §9-13 |

**端侧链（窄而深，建议主线）**：

```text
ONNX ──► TensorRT ──► CUDA / Triton
(格式)   (引擎)       (底层算子)
```
...
```text
vLLM / SGLang / TensorRT-LLM ──► 调度 / 批处理 / 分布式
```

> 定位提示：TensorRT 是「引擎」不是「服务」，端侧和数据中心都用，但端侧几乎绕不开它；vLLM/SGLang 是「在线服务框架」，和端侧是两条路。近期可盯死端侧链，在线服务先理解 PagedAttention、Continuous Batching、KV Cache 的原理即可，不必啃源码。
