...
项目起步阶段，使用`\init`命令来构建AGENTS.md，一个好的AGENTS.md可以分为下面几部分：
...
### 2.1 skills / plugins / extensions 的区别
...
Superpowers 是 CodeAgent 的核心技能集合，提供工作流规范和最佳实践。
...
**触发时机**：有 spec 或需求文档的多步骤任务，在接触代码之前
...
Hook 机制是基于插件系统实现的强大扩展能力，允许用户在关键节点拦截、修改和扩展 CodeAgent CLI 的核心行为。

### 3.1 Hook 类型

1. **Prompt Hook** - 在 AI 生成代码前后的拦截和处理
2. **工具 Hook** - 对 bash 等工具执行的前后处理
3. **认证 Hook** - 支持多种认证插件的扩展
4. **事件 Hook** - 全局事件监听和处理
5. **会话 Hook** - 基于会话上下文的定制化处理

### 3.2 Hook 文件基本结构

Hook 是一个标准的 TypeScript/JavaScript 模块，导出一个符合 Plugin 接口的函数。

```javascript
export const MyPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    // Hook 实现放在这里
    "chat.message": async (ctx, out) => {
      // Hook 逻辑
    },
...
interface HookOutput {
  // 具体的输出字段，根据 Hook 类型而异
}
```

### 3.4 示例：使用 Hook 执行定时任务

#### Plugin Hook vs Cron Job
...
```javascript
// 在 OpenCode 中创建 cronjob
cronjob({
  action: 'create',
  name: 'daily-code-summary',
  prompt: '请执行每日代码总结，生成今日工作报告',
  schedule: '0 30 17 * * *',  // 每天 17:30
  timezone: 'Asia/Shanghai',
  agent: 'general'
})
```
...
export const DailySummaryPlugin = async ({ client }) => {
  return {
    config: async (config) => {
      // 每分钟检查一次
      timer = setInterval(() => {
...
完整的定时任务实现示例，位于 `.opencode/plugins/daily-summary-hook.js`：

```javascript
/**
 * Daily Summary Hook Plugin
 * 每日代码总结，定时分析 git 提交记录，生成工作总结报告
 */

export const DailySummaryPlugin = async ({ client, directory }) => {
  return {
    // 配置 hook：注册 skills 路径
    config: async (config) => {
    ...
    // 消息转换 hook：注入 bootstrap 内容
    'chat.message.transform': async (_input, output) => {
      // 注入逻辑...
    }
  };
};
...
使用 SDD-Spec-Writer 来构建规范文件，SDD（Specification Driven Development）是一种规格驱动的开发方法。

### 4.1 SDD 工作流

```
需求 → TR2（需求分析）→ TR3（软件设计）→ 实现 → 测试
```
...
| Skill | 用途 |
|-------|------|
| `design-state-machine` | SDD HLD 流程状态机 |
| `sdd-track-manager` | track 状态跟踪管理 |
| `knowledge-trace` | 追溯设计文档的知识来源 |
| `plantuml-builder` | 生成 PlantUML 图表 |
| `datacom-design-asset-search` | 查询 AR 详情、组件规格 |

### 4.3 Datacom Extensions

| Extension | 用途 |
|-----------|------|
| `datacom-sdd` | 规格驱动开发（SDD）工作流 |
| `datacom-sdd-build` | CICD 构建集成 |
| `datacom-sdd-dt` | 开发者测试（DT）|
| `datacom-code-reviewer` | AI 代码检视 |
...
| 层级 | 原因 | 表现 |
|------|------|------|
| **注意力机制** | Softmax sink + 距离衰减 | 远处信息被稀释 |
| **位置编码** | 外推泛化能力有限 | 位置区分度下降 |
| **训练分布** | 超长序列训练不足 | 模型不熟悉这种输入 |
| **硬件限制** | KV Cache 压力大 | 可能触发量化/丢弃 |

**最佳实践**：将上下文保持在 50% 利用率以下，或使用 RAG 等技术。
...
1. **Hook 开发**：参考 `superpowers.js` 模板
2. **Skill 编写**：遵循 SKILL.md 格式
3. **定时任务**：使用 Cron Job 实现

### 6.4 性能优化

1. Skills 按需加载
2. 消息压缩长对话
3. 保持 context 利用率在 50% 以下
...
.git仓库/.opencode/         ← 项目级配置
├── plugins/
│   └── daily-summary-hook.js  ← 每日总结插件
├── skills/
│   └── daily-summary/
│       └── SKILL.md        ← 每日总结 skill
├── daily-summaries/        ← 报告输出目录
├── config.json             ← 配置文件
