...
静态转换`static_cast`是将一种数据类型的值强制转换为另一种数据类型的值。静态转换通常用于比较类型相似的对象之间的转换，例如将 int 类型转换为 float 类型。静态转换在编译时完成，不进行任何运行时类型检查，因此可能会导致运行时错误。
...
- `expression` 是要转换的表达式，必须是一个指针或引用类型，并且必须指向一个多态类型（即至少有一个**虚函数**的类）。
...
常量转换`const_cast`用于将 const 类型的对象转换为非 const 类型的对象。常量转换只能用于转换掉 const 属性，不能改变对象的类型。

常量类型转换是**唯一能增删 const/volatile 属性**的转换，不改变类型本身。

#### 重新解释类型转换

重新解释转换`reinterpret_cast`将一个数据类型的值重新解释为另一个数据类型的值，通常用于在不同的数据类型之间进行转换。重新解释转换不进行任何类型检查，因此可能会导致未定义的行为。

> 不使用C风格强制类型转换的原因？
> C 风格强制转换会**在 static/const/reinterpret 之间隐式选择一个最弱的**，意图不明确、编译器不检查、容易被重构/模板破坏，还难以用 grep 搜索定位；C++ 四种 cast 各自语义明确、可检索、可被编译器静态检查。
...
堆是一种**完全二叉树**结构，满足堆序性质（Heap Property）：任意节点的值都大于等于（大顶堆）或小于等于（小顶堆）其所有子节点的值。C++ 中的 `priority_queue` 就是堆的封装，常用于优先队列、Top-K 问题、堆排序、Dijkstra 最短路等场景。

特点：

- 大顶堆的堆顶（根节点）是最大值，小顶堆的堆顶是最小值。
- 插入、删除堆顶的时间复杂度为 O(log n)，取堆顶为 O(1)。
- 完全二叉树可用数组紧凑存储，无需指针。

数组索引关系（根在下标 0）：节点 i 的左孩子为 `2i+1`，右孩子为 `2i+2`，父节点为 `(i-1)/2`。

优缺点：

- 优点：动态维护一组元素的最值，效率远高于每次排序。
- 缺点：只能高效访问堆顶，不支持任意位置的随机访问或查找。
...
minHeap.push(1);
minHeap.push(5);
int minTop = minHeap.top(); // 1，最小值在堆顶
```
...
多线程实现示例：实现两个函数，各自输出从0-100的奇数和偶数，使用多线程实现终端连续输出0-100。
