...
|指令|描述|使用示例|
|---|---|---|
|`#define`|定义宏（符号常量或函数式宏）|`#define PI 3.1415`、`#define MAX(a,b) ((a) > (b) ? (a) : (b))`|
|`#include`|包含头文件|`#include <stdio.h>`、`#include "myheader.h"`|
|`#undef`|取消已定义的宏|`#undef PI`|
|`#ifdef`|如果宏已定义则编译后续代码|`#ifdef DEBUG` ... `#endif`，见下方示例|
|`#ifndef`|如果宏未定义则编译后续代码（常用于头文件保护）|`#ifndef HEADER_H` ... `#endif`，见下方示例|
|`#if`|条件编译（可配合defined操作符使用）|`#if VERSION > 2` ... `#endif`，见下方示例|
|`#else`|`#if`/`#ifdef`/`#ifndef`的替代分支|见下方示例|
|`#elif`|类似于else if|见下方示例|
|`#endif`|结束条件编译块|见下方示例|
|`#error`|产生编译错误并输出消息|`#error "需要C99标准"`，见下方示例|
|`#pragma`|编译器特定指令（非标准，各编译器不同）|`#pragma once`、`#pragma pack(1)`|

```c
/* 1. 条件编译：定义了 DEBUG 才编译调试语句 */
#ifdef DEBUG
